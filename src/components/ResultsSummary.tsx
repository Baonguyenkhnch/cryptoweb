...
export function ResultsSummary({ data, onLoginClick }: ResultsSummaryProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Main Score Card */}
      <Card className="relative overflow-hidden bg-gradient-to-br from-slate-800/90 to-slate-900/90 backdrop-blur-xl border border-cyan-500/30 shadow-2xl rounded-2xl">
        ...
        <CardContent className="relative p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="relative">
              <div className="w-40 h-40 rounded-full bg-gradient-to-br from-slate-700/50 to-slate-800/50 flex items-center justify-center border-4 border-cyan-500/30 shadow-xl">
                <div className="text-center">
                  <div className="text-5xl text-white mb-1">{data.score}</div>
                  <div className="text-xs text-gray-400">/ 850</div>
                </div>
              </div>
              <div className={`absolute -bottom-2 left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r ${rating.color} text-white text-sm shadow-lg`}>
                {rating.text}
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-gray-300">{t.resultsSummary.creditScore}</span>
                    <span className="text-cyan-400">{scorePercentage.toFixed(0)}%</span>
                  </div>
            ...
            <div className="relative space-y-2">
              {[
                ...
                <div key={index} className="relative">
                  <div className="flex items-center gap-3 p-3 bg-slate-700/30 rounded-lg border border-slate-600/30 hover:border-cyan-500/40 transition-all">
                    <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 rounded-lg flex items-center justify-center border border-cyan-500/30">
                      <span className="text-lg">{step.icon}</span>
                    </div>
                    <div className="flex-1">
                      <div className="text-sm text-gray-300">{step.label}</div>
                      <div className={`text-xs text-${step.color}-400`}>{step.value}</div>
                    </div>
                    <div className="text-cyan-400 text-xl">→</div>
                  </div>
                  {index < 2 && (
                    <div className="absolute left-7 top-full w-0.5 h-2 bg-gradient-to-b from-cyan-500/50 to-transparent"></div>
                  )}
                </div>
              ))}
            </div>
          ...
          <CardContent className="pb-4">
            ...
            <div className="space-y-2 mt-2">
              <div className="flex items-center gap-2 px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg">
                ...
                <div className="flex-1">
                  <div className="text-xs text-gray-300">{language === 'vi' ? 'Tính điểm tín dụng' : 'Calculate credit score'}</div>
                  <div className="text-xs text-green-400">✅ {language === 'vi' ? 'Hoàn thành' : 'Completed'}</div>
                </div>
              </div>
              <div className="flex items-center gap-2 px-3 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded-lg ring-1 ring-cyan-500/20">
                <div className="w-3 h-3 rounded-full bg-cyan-500"></div>
                <div className="flex-1">
                  <div className="text-xs text-gray-300">{language === 'vi' ? 'Đăng nhập xem chi tiết' : 'Login for details'}</div>
                  <div className="text-xs text-cyan-400">🔓 {language === 'vi' ? 'Bước tiếp theo' : 'Next step'}</div>
                </div>
              </div>
              <div className="flex items-center gap-2 px-3 py-2 bg-slate-700/20 border border-slate-600/20 rounded-lg">
                <div className="w-3 h-3 rounded-full bg-indigo-500"></div>
                <div className="flex-1">
                  <div className="text-xs text-gray-400">{language === 'vi' ? 'Dashboard & phân tích' : 'Dashboard & analysis'}</div>
                  <div className="text-xs text-purple-400">📈 {language === 'vi' ? 'Sắp mở khóa' : 'Coming soon'}</div>
                </div>
              </div>
            </div>
          </CardContent>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  <th className="text-left py-3 px-4 text-sm text-gray-400">{language === 'vi' ? 'Chỉ Số' : 'Metric'}</th>
                  <th className="text-right py-3 px-4 text-sm text-gray-400">{language === 'vi' ? 'Giá Trị' : 'Value'}</th>
                  <th className="text-right py-3 px-4 text-sm text-gray-400">{language === 'vi' ? 'Đánh Giá' : 'Assessment'}</th>
                </tr>
              </thead>
              <tbody>
