...
export function Login({ onRegisterSuccess, onBackToCalculator }: LoginProps) {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#0f1419] flex items-center justify-center p-4">
      ...
      <div className="absolute top-2.5 md:top-4 left-2.5 md:left-4 right-2.5 md:right-4 z-20 flex items-center justify-between">
        <Button
          onClick={onBackToCalculator}
          variant="outline"
          className="bg-slate-800/80 backdrop-blur-sm border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 hover:border-cyan-400/50 rounded-lg h-7 md:h-9 px-2.5 md:px-4 text-[11px] md:text-sm"
        >
      ...
      <div className="relative z-10 w-full max-w-md px-3">
        ...
        <div className="text-center mb-3">
          ...
          <h1 className="text-lg md:text-xl mb-1 bg-gradient-to-r from-cyan-400 via-blue-400 to-teal-400 bg-clip-text text-transparent leading-tight">
            {t.auth.registerTitle}
          </h1>
          <p className="text-gray-400 text-[11px] md:text-xs">{t.auth.registerSubtitle}</p>
        </div>
        ...
        <Card className="relative overflow-hidden bg-slate-800/50 backdrop-blur-xl border border-cyan-500/20 shadow-2xl rounded-xl">
          ...
          <div className="relative">
            ...
            <CardContent className="space-y-2 md:space-y-2.5 pb-3 px-3 md:px-6">
              {/* Alert - COMPACT */}
              {error && (
              ...
              <div className="p-2 md:p-2.5 bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-purple-500/10 border border-purple-500/20 rounded-lg">
                <div className="text-[10px] md:text-[11px] text-purple-300 mb-0.5 md:mb-1 font-medium">
                  {t.auth.benefits}
                </div>
                <ul className="space-y-0.5 text-[9px] md:text-[10px] text-gray-300">
              ...
              <form onSubmit={handleRegister} className="space-y-2 md:space-y-2.5">
                {/* Email - INLINE ERROR */}
                <div className="space-y-0.5 md:space-y-1">
                  <Label htmlFor="register-email" className="text-gray-300 flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs">
                    <Mail className="w-3 h-3 md:w-3.5 md:h-3.5 text-cyan-400" />
                    Email
                  </Label>
                  <Input
                    id="register-email"
                    ...
                    className={`bg-slate-900/50 border h-9 text-xs md:text-sm transition-colors ${hasAttemptedSubmit && !isValidEmail(registerEmail)
                      ? 'border-red-500/50 focus:border-red-400'
                      : 'border-cyan-500/30 focus:border-cyan-400'
                      } text-white placeholder:text-gray-500`}
                  />
                  {hasAttemptedSubmit && !isValidEmail(registerEmail) && (
                    <p className="text-[9px] md:text-[10px] text-red-400">Vui lòng nhập email hợp lệ</p>
                  )}
                </div>

                {/* Wallet - INLINE ERROR */}
                <div className="space-y-0.5 md:space-y-1">
                  <Label htmlFor="register-wallet" className="text-gray-300 flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs">
                    <Wallet className="w-3 h-3 md:w-3.5 md:h-3.5 text-teal-400" />
                    Địa Chỉ Ví
                  </Label>
                  <div className="flex gap-1 md:gap-1.5">
                    <Input
                      ...
                      className={`flex-1 bg-slate-900/50 border h-9 text-xs md:text-sm transition-colors ${hasAttemptedSubmit && !isValidWallet(registerWallet)
                        ? 'border-red-500/50 focus:border-red-400'
                        : 'border-cyan-500/30 focus:border-cyan-400'
                        } text-white font-mono text-[10px] md:text-[11px] placeholder:text-gray-600`}
                    />
                    <Button
                      type="button"
                      onClick={() => setRegisterWallet(generateRandomWallet())}
                      variant="outline"
                      className="bg-gradient-to-r from-teal-600/20 to-cyan-600/20 border-teal-500/40 text-teal-400 hover:bg-teal-500/30 px-2 md:px-2.5 h-9"
                      title="Tạo ví test"
                    >
                ...
                <div className="space-y-0.5 md:space-y-1">
                  <Label htmlFor="register-password" className="text-gray-300 flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className={`bg-slate-900/50 border pr-9 h-9 text-xs md:text-sm transition-colors ${hasAttemptedSubmit && !isValidPassword(registerPassword)
                        ? 'border-red-500/50 focus:border-red-400'
                        : 'border-cyan-500/30 focus:border-cyan-400'
                        } text-white placeholder:text-gray-500`}
                    />
                    ...
                    >
                      {showRegisterPassword ? <Eye className="w-3 h-3 md:w-3.5 md:h-3.5" /> : <EyeOff className="w-3 h-3 md:w-3.5 md:h-3.5" />}
                    </button>
                  </div>
                  {hasAttemptedSubmit && !isValidPassword(registerPassword) && (
                    <p className="text-[9px] md:text-[10px] text-red-400">Mật khẩu phải có ít nhất 6 ký tự</p>
                  )}
                </div>

                {/* Confirm Password - INLINE ERROR */}
                <div className="space-y-0.5 md:space-y-1">
                  <Label htmlFor="register-confirm" className="text-gray-300 flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs">
                    <Lock className="w-3 h-3 md:w-3.5 md:h-3.5 text-blue-400" />
                    Xác Nhận Mật Khẩu
                  </Label>
                  <div className="relative">
                    <Input
                      ...
                      className={`bg-slate-900/50 border pr-9 h-9 text-xs md:text-sm transition-colors ${hasAttemptedSubmit && registerConfirmPassword && !passwordsMatch
                        ? 'border-red-500/50 focus:border-red-400'
                    ...
                    <button
                      ...
                      className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-cyan-400"
                    >
                      {showConfirmPassword ? <Eye className="w-3 h-3 md:w-3.5 md:h-3.5" /> : <EyeOff className="w-3 h-3 md:w-3.5 md:h-3.5" />}
                    </button>
                  </div>
                  {hasAttemptedSubmit && registerConfirmPassword && !passwordsMatch && (
                    <p className="text-[9px] md:text-[10px] text-red-400">Mật khẩu không khớp</p>
                  )}
                  {hasAttemptedSubmit && passwordsMatch && (
                    <p className="text-[9px] md:text-[10px] text-green-400 flex items-center gap-1">
                      <CheckCircle2 className="w-2.5 h-2.5 md:w-3 md:h-3" /> Khớp!
                    </p>
                  )}
                </div>
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 hover:from-blue-500 hover:via-purple-500 hover:to-blue-500 text-white h-9 md:h-10 mt-2 md:mt-3 text-xs md:text-sm font-medium shadow-lg shadow-purple-500/30"
                >
                  {isLoading ? (
